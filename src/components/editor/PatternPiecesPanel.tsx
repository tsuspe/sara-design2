...
import type { PatternPiece, Page3Technical } from '@/types'
...
interface PatternPiecesPanelProps {
  page: Page3Technical
  onUpdate: (pieces: PatternPiece[]) => void
}

export default function PatternPiecesPanel({ page, onUpdate }: PatternPiecesPanelProps) {
  ...
  const handleUpdate = (id: string, changes: Partial<PatternPiece>) => {
    onUpdate(pieces.map((p) => (p.id === id ? { ...p, ...changes } : p)))
  }

  const handleDelete = (id: string) => {
    onUpdate(pieces.filter((p) => p.id !== id))
  }
